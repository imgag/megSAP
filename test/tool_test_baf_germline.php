...
check_exec("php ".src_folder()."/Tools/{$name}.php -name vc_strelka2_tu_in -out $out1 -vcf $vcf --log ".output_folder()."{$name}_1.log");
check_file($out1, data_folder()."{$name}_out1.igv");

// test3: dragen input
$vcf = data_folder()."/{$name}_in3.vcf.gz";
$out2 = output_folder()."{$name}_out2.igv";
check_exec("php ".src_folder()."/Tools/{$name}.php -name NA12878_03 -out $out2 -vcf $vcf --log ".output_folder()."{$name}_1.log");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -name vc_strelka2_tu_in -out $out3 -vcf $vcf -downsample 10 --log ".output_folder()."{$name}_2.log");
