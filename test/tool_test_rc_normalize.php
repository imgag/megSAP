...
$in_file_gz = data_folder().$name."_in.tsv.gz";
$tmp_file = temp_file($name."_in.tsv");
exec2("gunzip -c $in_file_gz > $tmp_file");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -in '$tmp_file' -out '$out_file1' -method fpkm --log $log_file1");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -in '$tmp_file' -out '$out_file2' -method raw,cpm --log $log_file2");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -in '$tmp_file' -out '$out_file3' -method raw,cpm,fpkm,tpm --log $log_file3");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -in '$tmp_file' -in_exon '$in_file_exon' -out '$out_file4' -out_exon '$out_file4_exon' -method raw,cpm,fpkm,tpm --log $log_file4");
