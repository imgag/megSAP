...
check_exec("php ".src_folder()."/Tools/{$name}.php -in_fastq {$in1_file} -system {$in1_system} -out {$out_file1} --log ".output_folder().$name."_out1.log -threads 1 -bam_output");
check_file($out_file1, data_folder().$name."_out1.bam");

//bam input
$out_file2 = output_folder().$name."_out2.bam";
check_exec("php ".src_folder()."/Tools/{$name}.php -in_bam {$in2_file} -system {$in1_system} -out {$out_file2} --log ".output_folder().$name."_out2.log -threads 1 -bam_output");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -in_bam {$in2_file} -system {$in1_system} -out {$out_file3} --log ".output_folder().$name."_out2.log -threads 1");
