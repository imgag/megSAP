...
function column_correlation($tsv1, $tsv2, $col1, $col2)
{
	...
	$corr = correlation($table1->getCol($table1->getColumnIndex($col1)),
		$table2->getCol($table2->getColumnIndex($col2)));
	return($corr);
}
...
check(column_correlation($out_file1, data_folder().$name."_out1.tsv", "TPM", "TPM"), 1.0, 0.005);
...
//check TPM correlation, at least 0.995
check(column_correlation($out_file2, data_folder().$name."_out2.tsv", "TPM", "TPM"), 1.0, 0.005);
