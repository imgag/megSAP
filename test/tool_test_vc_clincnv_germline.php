...
check_exec("php ".src_folder()."/Tools/{$name}.php -cov {$cov_folder}/DX000018_02.cov.gz -cov_folder {$cov_folder} -cov_min 20 -bed {$bed} -out {$out_file1} -mosaic --log {$log_file1}");
...
check_exec("php ".src_folder()."/Tools/{$name}.php -cov {$cov_folder}/DX000015_01.cov -cov_folder {$cov_folder} -cov_max 200 -max_cnvs 200 -bed {$bed} -bed_off {$bed_off} -cov_off {$cov_off} -out {$out_file3} --log {$log_file3} -tumor_only");
