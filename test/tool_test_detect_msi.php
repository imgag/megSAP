...
//with target
$target = data_folder()."detect_msi_in.bed";
$out = output_folder().$name."_out3.tsv";
check_exec("php ".src_folder()."/Tools/{$name}.php -n_bam $in_n -t_bam $in_t -msi_ref $msi_ref -out $out -target $target");
check_file($out, data_folder().$name."_out3.tsv", true);
