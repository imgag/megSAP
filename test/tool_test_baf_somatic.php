...
check_exec("php ".src_folder()."/Tools/baf_somatic.php -bam_t {$t_bam} -bam_n {$n_bam} -vcf {$vcf} -out {$out1} -depth --log {$log1}");
check_file($out1, data_folder().$name."_out1.igv");

//test 2 - downsampling
$out2 = output_folder().$name."_out2.igv";
$log2 = output_folder().$name."_out2.log";
check_exec("php ".src_folder()."/Tools/baf_somatic.php -bam_t {$t_bam} -bam_n {$n_bam} -vcf {$vcf} -out {$out2} -depth -downsample 2 --log {$log2}");
check_file($out2, data_folder().$name."_out2.igv");

$out3 = output_folder().$name."_out3.igv";
$log3 = output_folder().$name."_out3.log";
check_exec("php ".src_folder()."/Tools/baf_somatic.php -bam_t {$t_bam} -bam_n {$n_bam} -vcf {$vcf} -out {$out3} -depth -downsample 3 --log {$log3}");
