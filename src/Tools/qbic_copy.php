...
foreach($res as $row)
{
	...
	foreach($res2 as $row2)
	{
		...
		if ($normal_id!="")
		{
			...
			{
				$pattern = "{".$data_folder."/{$ps_name}-{$ps_name2}*_var_annotated.vcf.gz,".$data_folder."/{$ps_name}-{$ps_name2}*.GSvar}"; #,".$data_folder."/{$ps_name2}*_var_annotated.vcf.gz,".$data_folder."/{$ps_name2}.GSvar
			}
