...
# 
# script modified in 2025 to also support DRAGEN structural varaint calls
#
...
def convertInversions(samtools, refFasta, vcfFile, invMateDict, dragen=False):
    ...
    for line in fpVcf:
        ...
        if vcfRec.isINV3 or vcfRec.isINV5:
            ...
            # update manta ID
            vidSuffix = vcfRec.vid.split(bnd)[1]
            idx = vidSuffix.rfind(':')
            vcfRec.vid = inv + "%s" % vidSuffix[:idx]
            ...
            for infoItem in vcfRec.info:
                ...
                # update event ID
                    ...
                    eidSuffix = vcfRec.infoDict["EVENT"].split(bnd)[1]
                    idx = vidSuffix.rfind(':')
                    infoEventStr = "EVENT=" + inv + "%s" % eidSuffix[:idx]
...
if __name__=='__main__':
    ...
    usage = "convertInversion.py <samtools path> <reference fasta> <vcf file> [manta|dragen]\n"
    if len(sys.argv) <= 3:
    ...
    if (mode != "manta") and (mode != "dragen"):
        sys.stderr.write("Invalid mode provided! Please provide either 'manta' or 'dragen'.")
        sys.stderr.write(usage)
        sys.exit(1)
    ...
    convertInversions(samtools, refFasta, vcfFile, invMateDict, dragen)
