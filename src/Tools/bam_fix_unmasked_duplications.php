...
if ($remap_missing_chr)
{
	...
	elseif ($exit_code == 1)
	{
		...
		$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "BedAdd", "-in {$reg} {$tmp_bed_missing_chr} -out {$merged_bed}", [$reg]);
...
$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "BamExtract", "-in {$in} -ref {$in_ref} -ids {$read_ids} -out {$bam_reg} -out2 {$bam_other}", [$in, $in_ref], [$bam_reg, $bam_other]);
print "  took ".time_readable(microtime(true)-$time_start)."\n";
...
$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "BamToFastq", "-in {$bam_reg} -out1 {$fastq1} -out2 {$fastq2}", [$bam_reg], [$fastq1, $fastq2]);
print "  took ".time_readable(microtime(true)-$time_start)."\n";
...
$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "BedHighCoverage", "-bam {$bam_mapped} -out {$remapped_region} -cutoff 1 -threads {$threads}", [$bam_mapped], [$remapped_region]);
print "  took ".time_readable(microtime(true)-$time_start)."\n";
...
$time_start = microtime(true);
print "merging BAMs\n";
$parser->execSingularity("samtools", get_path("container_samtools"), "samtools merge", "--write-index -@ {$threads} -f -h {$bam_mapped} --reference {$ref} -o {$merged_bam} {$bam_mapped} {$bam_other}", [$bam_mapped, $ref, $bam_other], [$merged_bam]);
print "  took ".time_readable(microtime(true)-$time_start)."\n";

if ($replace_readgroup)
{
	print "replace RG\n";
	list($in_header) = $parser->execSingularity("samtools", get_path("container_samtools"), "samtools view", "-H {$merged_bam}", [$merged_bam]);
	
	//extract @SQ lines from mapped file
	list($in_header2) = $parser->execSingularity("samtools", get_path("container_samtools"), "samtools view", "-H {$bam_mapped}", [$bam_mapped]);
	$new_sq_lines = array();
	...
	$parser->execSingularity("samtools", get_path("container_samtools"), "samtools addreplacerg", "-@ {$threads} -R {$sample} -m overwrite_all -o {$tmp_fixed_bam} {$merged_bam}", [$merged_bam]);
	$tmp_fixed_bam2 = $debug ? $out_folder."/fixed_rg_header.bam" : $parser->tempFile("_fixed_header.bam");
	$parser->execSingularity("samtools", get_path("container_samtools"), "samtools reheader", "{$tmp_header} {$tmp_fixed_bam} > {$tmp_fixed_bam2}", [], [$tmp_fixed_bam2]);
	$parser->execSingularity("samtools", get_path("container_samtools"), "samtools view", "-h --write-index -@ {$threads} -T {$ref} -C -o {$out} {$tmp_fixed_bam2}", [$ref, $tmp_fixed_bam2], [$out]);
	print "  took ".time_readable(microtime(true)-$time_start)."\n";
}
