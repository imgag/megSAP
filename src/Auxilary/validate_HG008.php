...
$parser = new ToolBase("validate_HG008", "Validates the performance of a sequencing experiment on the GiaB tumor reference sample HG008.");
...
function get_variants($vcf_gz, $roi, $max_indel, $min_qual, &$skipped)
{
	...
	foreach($file as $line)
	{
		...
		if (starts_with(strtolower($line), "#chrom")) $header = explode("\t", $line);
...
/* 		//skip low mappabilty variants (megSAP)	//TODO remove if not neccesssary
...
$roi_used = $roi_hc;
$bases_used = $bases_hc;
if ($min_dp>0) //TODO Kilian check if it works for HG008 too
{
