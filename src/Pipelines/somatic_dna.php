...
if (in_array("an", $steps))
{
	...
	if (!$single_sample)
	{
		...
		$args_somaticqc = [
			...
			"-blacklist", get_path("data_folder")."/gene_lists/somatic_tmb_blacklist.bed", //Blacklisted genes that are not included in TMB calculation (e.g. HLA-A and HLA-B)
			"-tsg_bed", get_path("data_folder")."/gene_lists/somatic_tmb_tsg.bed", //TSG genes whose mutations are treated specially in TMB calculation
			"-ref", $ref_genome,
			"-out", $somaticqc
		];
		if (!empty($links))
