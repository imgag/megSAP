...
if (in_array("ma", $steps))
{
	...
	{
		list($stdout, $stderr) = $parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "FastqFormat", "-in {$file}", [$file]);
	...
	if (in_array($sys['umi_type'], ["IDT-UDI-UMI"]))
	{
		if (empty($in_umi))
		{
			...
			list($stdout, $stderr, $return_code) = $parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "FastqCheckUMI", "-in ".$in_for[0], [$in_for[0]]);
		...
		else
		{
			...
			$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "FastqAddBarcode", "-in1 ".implode(" ", $in_for)." -in2 ".implode(" ", $in_rev)." -in_barcode ".implode(" ", $in_umi)." -out1 $merged1_bc -out2 $merged2_bc", [$folder]);
	...
	$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "SeqPurge", implode(" ", $seqpurge_params), [$folder], [$out_folder]);
	...
	if (in_array("ma", $steps))
	{
		if ($filter_hb)
		{
			...
			$parser->execSingularity("kraken2", get_path("container_kraken2"), "kraken2", implode(" ", $kraken_args), [get_path("data_folder")."/dbs/kraken2_filter_hb"]);
		...
		if ($umi && !$skip_dedup)
		{
			...
			$pipeline[] = ["", $parser->execSingularity("samtools", get_path("container_samtools"), "samtools fixmate", "-@ {$threads} -u -m - -", [], [], 1, true)];
			$tmp_for_sorting2 = $parser->tempFile();
			//sort by coordinate
			$pipeline[] = ["", $parser->execSingularity("samtools", get_path("container_samtools"), "samtools sort", "-T {$tmp_for_sorting2} -m 2G -@ {$threads}", [], [], 1, true)];
			$tmp_for_markdup = $parser->tempFile();
			//mark duplicates by UG tag (unique group from umi_tools group)
			$pipeline[] = ["", $parser->execSingularity("samtools", get_path("container_samtools"), "samtools markdup", "-T {$tmp_for_markdup} -c --barcode-tag UG -s -@ {$threads} - {$final_bam}", [], [$out_folder], 1, true)];
		...
		$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "MappingQC", implode(" ", $mappingqc_params), $in_files);
	}
...
if (in_array("rc", $steps))
{
	...
	$parser->execSingularity("subread", get_path("container_subread"), "repair", "-i {$final_bam} -o {$repair_bam} -T {$threads}", [$out_folder]);
	...
	if (db_is_enabled("NGSD"))
	{
		...
		if (!is_null($ps_info))
		{
			if ($build=="GRCh38")
			{
				...
				$in_files[] = $out_folder;
				$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "NGSDAnnotateRNA", "-mode genes -update_genes -ps {$name} -cohort_strategy {$cohort_strategy} -in {$counts_normalized} -out {$expr} -corr {$expr_corr} {$hpa_parameter}", $in_files);
				$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "NGSDAnnotateRNA", "-mode exons -update_genes -ps {$name} -cohort_strategy {$cohort_strategy} -in {$counts_exon_normalized} -out {$expr_exon}", [$out_folder]);
			}
		...
		{
			$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "SplicingToBed", "-in {$junctions} -report {$splicing_annot} -gene_report {$splicing_gene} -bed {$splicing_bed}", [$out_folder]);
		}
	}
}
...
if (in_array("ma", $steps) || in_array("rc", $steps) || in_array("an", $steps))
{
	...
	if (file_exists($expr))
	{
		...
		//run RnaQC
		$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "RnaQC", implode(" ", $args), $in_files);
}
...
if (in_array("plt", $steps))
{
	...
	$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "NGSDExtractRNACohort", "-genes {$all_gene_file} -ps {$name} -sample_expression {$expr} -cohort_strategy {$cohort_strategy} -out {$expr_cohort}", [$out_folder]);
...
if (in_array("db", $steps))
{
	...
	$parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "NGSDImportSampleQC", "-ps $name -files ".implode(" ", $qc_file_list)." -force", [$out_folder]);
	...
	if ($build=="GRCh38")
	{
		if (file_exists($expr)) $parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "NGSDImportExpressionData", "-mode genes -expression {$expr} ".implode(" ", $args), [$out_folder]);
		if (file_exists($expr_exon)) $parser->execSingularity("ngs-bits", get_path("container_ngs-bits"), "NGSDImportExpressionData", "-mode exons -expression {$expr_exon} ".implode(" ", $args), [$out_folder]);
	}	
}
