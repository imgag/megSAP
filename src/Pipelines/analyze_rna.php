...
if (in_array("fu",$steps))
{
	...
	if (is_dir($fusion_index)) {
		...
		$fusion_tmp_folder = $prefix . "_fusions";
...
//		$input_reads_available = true;
		if ($input_reads_available)
		{
			$starfusion_params[] = "--FusionInspector inspect";
//			$starfusion_params[] = "--left_fq ". implode(" ", $in_for);
			$starfusion_params[] = "--left_fq ". $fastq_trimmed1;
			if ($paired)
			{
//				$starfusion_params[] = "--right_fq " . implode(" ", $in_rev);
		...
		if ($input_reads_available)
		{
			...
			$parser->execTool("NGS/igv_session.php", "-genome {$prefix}_var_fusions.fa -out {$igv_session_file} -in {$igv_tracks} -relative");
		}
	}
	else
	{
