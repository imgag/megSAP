...
exec($parser->execSingularity("arriba", get_path("container_arriba"), "arriba", implode(" ", $args)." 2>&1", [$bam, $genome, $gtf, $sv], [$out_fusions, $out_discarded], 1, true), $output, $exit_code);
...
if (isset($out_vcf)) {
    ...
    $parser->execSingularity("arriba", get_path("container_arriba"), "convert_fusions_to_vcf.sh", implode(" ", $args), [$genome, $out_fusions], [$out_vcf]);
}
...
if (isset($out_pdf)) {
    ...
    $parser->execSingularity("arriba", get_path("container_arriba"), "draw_fusions.R", implode(" ", $plot_args), [$gtf, $bam], [$out_pdf]);
...
if (isset($out_bam)) {
    ...
    if (count($reads) !== 0)
    {
        ...
        $parser->execSingularity("samtools", get_path("container_samtools"), "samtools", "view -H {$bam} > {$bam_header}", [$bam]);
        $parser->execPipeline([
            ["", $parser->execSingularity("samtools", get_path("container_samtools"), "samtools", "view -T {$genome} -O SAM {$bam}", [$genome, $bam], [], 1, true)],
            ["grep", "-F -f {$read_ids} > {$bam_records}"]
        ], "filter BAM");
        $parser->execPipeline([
            ["cat", "{$bam_header} ${bam_records}"],
            ["", $parser->execSingularity("samtools", get_path("container_samtools"), "samtools", "view -T {$genome} -o {$out_bam}", [$genome], [$out_bam], 1, true)]
        ], "write BAM");
