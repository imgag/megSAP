...
$parser->addInfile("vcf", "Variant list with SNPs to use (in VCF/VCF.GZ format).", true);
...
if ($handle === FALSE)
{
	trigger_error("Could not open file '{$vcf}' for reading.", E_USER_ERROR);
}

$handle_out = fopen($snps_filtered, "w");
if ($handle_out === FALSE)
{
	trigger_error("Could not open file '{$snps_filtered}' for writing.", E_USER_ERROR);
}
