...
list($exons) = $parser->exec(get_path("ngs-bits")."/Cidx", "-in ".get_path("data_folder")."/dbs/UCSC/exons.bed -pos $c:$s-$e", false);
$hits = array();
foreach ($exons as $exon)
{
	$hits[] = array_slice(explode("\t", $exon), 2); //start, end, gene_exon, strand
}
