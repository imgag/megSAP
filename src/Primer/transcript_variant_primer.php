...
$pipeline = [
	["", $parser->execApptainer("samtools", "samtools", "faidx $reference $transcript_id", [$reference], [], true)],
	["tail", "-n+2"],
	["tr", "-d '\n'"]
];
...
list($stdout, ) = $parser->execApptainer("blastn", "blastn", "-task blastn-short -db /mnt/storage3/users/ahmattj1/GRCh37_cDNA_blast -query {$primers_fasta} -outfmt 6 -evalue 0.1", ["/mnt/storage3/users/ahmattj1/"]);
...
$parser->execApptainer("STAR", "STAR", "--genomeLoad LoadAndKeep --genomeDir {$genome_dir} --outFileNamePrefix {$STAR_tmp_folder}/ --readFilesIn {$primers_fasta} --outSAMtype SAM --outStd SAM > {$star_out}", [$genome_dir]);
